"""
convert_wikilinks2.py
———————————————————————————————————————————————————————————————
Replace `process_line` to handle inline code, and process_file to handle the full line protection:
  convert_wikilinks.py
  +48
  -31
Теперь скрипт пропускает защищенные зоны: ```lang ... ``` и '[[...]]`. Логика простая: строка разбивается по инлайн-коду (всё между одиночными backticks), нечётные сегменты — код, чётные — текст. Wikilinks конвертируются только в чётных сегментах.
———————————————————————————————————————————————————————————————
...
def process_line(line: str) -> str:
    """
    Обрабатывает одну строку, конвертируя wikilinks.
    Пропускает содержимое инлайн-кода (`...`).
    """

    # Разбиваем строку на сегменты: нечётные — инлайн-код, чётные — обычный текст
    # Пример: "текст `код [[x]]` текст" → ["текст ", "код [[x]]", " текст"]
    segments = re.split(r'(`[^`]*`)', line)

    result = []
    for i, segment in enumerate(segments):
        if i % 2 == 1:
            # Нечётный индекс — это инлайн-код, не трогаем
            result.append(segment)
            continue

        # Чётный индекс — обычный текст, конвертируем wikilinks

        # 1. ![[image.png]] — изображения и embeds
        ...
        segment = re.sub(r'!\[\[([^\]]+)\]\]', replace_image_embed, segment)
        ...
        segment = re.sub(r'\[\[([^\]|]+)\|([^\]]+)\]\]', replace_aliased, segment)
        ...
        segment = re.sub(r'\[\[([^\]|]+)\]\]', replace_simple, segment)
